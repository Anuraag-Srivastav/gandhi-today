...
A bounded model review checks factual support, source scope, invented approval conditions and target relevance when source passages exist. Empty-evidence review was removed after live tests showed additional latency and unsupported absence claims without a demonstrated quality gain. Review issues share the existing one-rewrite budget with length correction. Model review is fallible, is not a historical accuracy guarantee, and adds a provider call. The corrected answer receives deterministic length validation; it is not subjected to an unbounded review loop.

The router receives explicit message indices and a strict output schema. Its selected assistant index identifies conversational context, not evidence, and can accompany historical or interpretive follow-ups as well as verification. Quotation-authentication turns must provide wording copied verbatim from conversation; missing or invented wording routes to clarification before research.
