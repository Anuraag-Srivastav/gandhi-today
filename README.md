...
V24 routing uses typed JSON (`historical`, `interpretation`, `definition`, `reading`, `reassessment`, `unrelated`) and fails explicitly on invalid output. It receives full conversational context as data, not answer examples. Routing has a ten-second deadline within the same 105-second overall limit. Source-bearing answers use Medium/temperature zero; browser research uses Low/2200 tokens following Groq's browser-search guidance. Search-free turns retain the selected effort with temperature 0.2. Application policies use the developer role documented for GPT-OSS Harmony. The per-turn policy follows history. Evidence retry receipts also cover historical generation failures. Unit tests establish request construction, not model compliance; live transcripts remain required.

Protocol references: https://developers.openai.com/cookbook/articles/openai-harmony and https://console.groq.com/docs/tool-use/built-in-tools/browser-search. The role and search-effort changes require live validation; documentation alone does not establish a quality improvement.

`v24-context3` binds retrieved evidence to its original question. A source follow-up on that exact answered question may reuse it (`retained-evidence`), without implying a fresh search or that all claims were supported. Explicit fresh-search requests bypass reuse. Retry reuse remains separately signed and request-bound. A small recent-conversation window accompanies research only to disambiguate its subject; final answer generation retains the full accepted history. These mechanisms contain no topic-specific answers.
