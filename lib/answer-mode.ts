...
export function turnInstruction(mode: ReturnType<typeof answerMode> | "historical" | "interpretation" | "definition" | "unrelated") {
  ...
  switch (mode) {
    ...
    case "interpretation": return shared + "Give a tentative interpretation, not a historical report. Explain one relevant Gandhian principle through the particular choice and its consequence. Do not add remembered claims that Gandhi praised, warned, wrote or endorsed something. A relevant concern is not an exclusive condition for approval: do not invent obligations or motives. If historical restrictions are established in the evidence, preserve them in the application rather than turning them into permissive modern advice. Stop after the explanation; no policy package, virtue list or concluding paraphrase.";
    ...
    case "reading": return shared + "Suggest at most two works whose identity and relevance are established by the retrieved evidence. Cite the bibliographic source. Label them background, not passage-level proof. No remembered essay titles, chapters, volumes, dates or page numbers. Do not imply the work proves the preceding answer. If the evidence establishes no suitable work, say that specifically rather than inventing one.";
