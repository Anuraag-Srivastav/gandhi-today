...
test("target must be an existing assistant, and only for a challenge", () => {
  ...
  expect(parseAnswerPlan(JSON.stringify({kind:"verification",question:"Check the earlier claim",targetIndex:1}), history).targetIndex).toBe(1);
  for (const targetIndex of [-1, 0, 4, 1.5, undefined]) {
    expect(() => parseAnswerPlan(JSON.stringify({kind:"verification",question:"Check",targetIndex}), history)).toThrow();
  }
  expect(() => parseAnswerPlan(JSON.stringify({kind:"definition",question:"Define",targetIndex:1}), history)).toThrow();
