...
test("quotation authentication cannot invent wording absent from conversation", () => {
  ...
  messages.push({role:"assistant",content:"An earlier historical claim."});
  expect(parseAnswerPlan(plan({kind:"verification",question:"Find the passage supporting your answer",targetIndex:1,quotationCheck:true,quotationText:null}), messages).kind).toBe("verification");
});
