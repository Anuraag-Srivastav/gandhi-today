...
export class AnswerLimitError extends Error {
  readonly validation;
  constructor(text: string, reason?: "provider-incomplete") {
    ...
    this.validation = { ...size, reason: reason || (!size.words ? "empty" : size.words > 140 ? "words" : "paragraphs") };
  }
}

/** Rewrite once with full context; callers must retain evidence and qualifications. */
export async function enforceAnswerLimits(text: string, rewrite: (draft: string) => Promise<string>, providerCompleted = true) {
  const valid = (answer: string) => {
    const size = answerSize(answer);
    return size.words > 0 && size.words <= 140 && size.paragraphs <= 3;
  };
  if (providerCompleted && valid(text)) return { text: text.trim(), rewritten: false };
