...
(async () => {
  ...
  try {
    for (const width of [390, 1280]) {
      ...
      await page.getByText('Test questions and settings', { exact: true }).click();
      await page.getByLabel('Answer reasoning', { exact: true }).selectOption('medium');
      await page.getByLabel('Load a question into the input').selectOption('Would Gandhi approve of competitive sport?');
      assert.equal(await input.inputValue(), 'Would Gandhi approve of competitive sport?');
      assert.equal(requests.length, 0, 'loading a test question does not send automatically');
      await input.fill('');
      ...
      assert.equal(requests[0].reasoningEffort, 'medium');
